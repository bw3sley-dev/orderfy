...
const allOrders=JSON.parse(localStorage.getItem('allOrders'))||[];const orderId=new Date().toISOString();const orderDate=new Date().toLocaleDateString('pt-BR');const total=currentOrder.reduce((sum,item)=>sum+item.subtotal,0);allOrders.push({id:orderId,date:orderDate,items:currentOrder,total,});localStorage.setItem('allOrders',JSON.stringify(allOrders));currentOrder=[];updateOrderTable();alert('Pedido finalizado com sucesso!')});const allOrdersTableBody=document.getElementById('all-orders-table').querySelector('tbody');const loadOrders=()=>{const allOrders=JSON.parse(localStorage.getItem('allOrders'))||[];allOrdersTableBody.innerHTML='';allOrders.forEach((order,index)=>{const items=order.items.map(item=>`${item.quantity}x ${item.name}`).join(', ');const row=`
    <tr>
      <td>${index + 1}</td>
      <td>${items}</td>
