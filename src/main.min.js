...
`;allOrdersTableBody.innerHTML+=row;document.getElementById(`print-order-${index}`).addEventListener('click',()=>{const zpl=generateZPL(index,order);printOrder(zpl)})})}
...
await device.claimInterface(0);const encoder=new TextEncoder();const data=encoder.encode(zpl);await device.transferOut(1,data);alert('Pedido enviado para a impressora com sucesso!')}catch(error){console.error('Erro ao se conectar à impressora:',error);alert('Erro ao enviar para a impressora. Verifique se o WebUSB é compatível.')}}
